...
st.subheader("**Prediction of Price Category**")
...
pref = st.text_input("Enter your Purchased product Price in US Dollars")
...
if st.session_state.clicked:
  ...
  try:
    data=pd.DataFrame({'year' : [year],'month' : [month],'day': [day],'order':[order],
                       ...
                       'model_photography':[photo],'price': [pref ],'page': [page]})
  except:
  ...
  if output == 0:
    st.subheader("Your Purchased product price is Below Average price")
  else:
    st.subheader("Your Purchased product price is Above Average price")

else:
  st.write('Kindly click Submit button to predict your Purchase Category!')
